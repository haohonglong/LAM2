...
## 一、主配置文件配置 参考 二、开发约定
		...
		为什么要像上面这样做呢？想一下如果我的所有静态都都写好了，但因需要多个项目共享一个LAM 类库（不需要每个项目里都在copy 一个LAM,当然你也可以这么做），这时要移动LAM文件夹位置,通常做法是修改每个页面里的script 的src 的路径，
		如果一两个还可以，但要是n个文件呢，是不是我们都要每个文件都要打开修改一遍呢，当然有些IDE 有批量修改的功能（这样批量修改还会意外的错误），想一下如果我们只修改一个地方就解决这个问题，是不是比批量修改更快呢？所以上面的配置文件就是解决这个问题的。
