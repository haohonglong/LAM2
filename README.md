...
## 二十二、预处理指令
  #### define：<#define NAME="value" />
    可在模版里定义常量，
    只接受字符串数据格式
    注意：必须单独占一行,属性值内绝对不能出现空格，否则正则匹配不到，因此报意外错误。
    模版变量会被解析
  ...
  ######  override="true:true" 这个可选属性代表blockid 发生冲突时，可以覆盖之前的block里存储的数据和内容,第一个ture 代表覆盖数据，第二个true代表覆盖内容，它们默认都是false(两者覆盖操作都不执行)。
  ...
  #### exec_script ： 
         ...
         * 如果不想在页面打印<script type="text/javascript"></script>,可以用一个删除标签包裹着它。（删除标签的用法参看delete指令）
         *
